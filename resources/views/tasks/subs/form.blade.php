    ...
    <div class="px-4 w-full pb-2 md:pb-4">
        <div class="flex relative flex-col mb-6 min-w-0 break-words bg-white rounded shadow-lg bg-blueGray-100 xl:mb-0">
            ...
            <div class="flex-auto p-4">
                ...
                <form action="{{ $state === 'Update' ? route('tasks.subs.update', ['task' => $task, 'sub' => $sub]) : route('tasks.subs.store', ['task' => $task]) }}" method="POST">
                    @csrf
                    @if ($state === 'Update')
                        @method('PUT')
                    @endif
                    ...
                    <div class="flex flex-wrap">
                        <div class="w-full px-4">
                            <div class="relative w-full mb-3">
                                <x-label for="task" :value="__('Parent Task')"/>
                                <select name="task_id" id="task" class="rounded-md w-full shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
