<x-app-layout>
    ...
    <div class="w-full px-4 pb-2 md:pb-4">
        ...
        @if ($team_tasks)
            <div class="relative flex flex-col min-w-0 break-words mb-5">
                <div class="flex-auto text-white text-xl font-semibold">
                    <div class="lg:w-1/2 w-full mb-6 lg:mb-0">
                        <h1 class="sm:text-3xl text-2xl font-medium title-font mb-1 text-blueGray-500">{{ __('Team Tasks') }}</h1>
                        <div class="h-1 w-20 bg-blueGray-500 rounded"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="overflow-y-scroll h-48">
                <div class="block w-full overflow-x-auto shadow-lg">
                    <table class="items-center w-full bg-white border-collapse">
                        <thead>
                            <tr>
                                <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center bg-blueGray-100 text-blueGray-500 border-blueGray-100">
                                    Name
                                </th>
                                <th class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-center bg-blueGray-100 text-blueGray-500 border-blueGray-100">
                                    Project
                        ...
                        <tbody>
                            @foreach($team_tasks as $team_task)
                                <tr>
                                    ...
                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $team_task->project->name }}
                                    </td>
                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $team_task->level->name }}
                                    </td>
                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $team_task->state->name }}
                                    </td>
                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        {{ $team_task->user->name }}
                                    </td>
                                    <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center">
                                        <a href="{{ route('tasks.subs.index', ['task' => $team_task]) }}" class="hover:underline">{{ $team_task->subs_count }}</a>
                                    </td>
                                    <td class="text-center border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                        @can('manage-sub-tasks', $team_task)
                                            <a href="{{ route('tasks.subs.create', ['task' => $team_task]) }}">
                                                <button class="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-xs px-2 py-1 rounded-full shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-150" type="button">
                                                    <i class="fas fa-plus"></i> {{ __('Create Sub Task') }}
                                                </button>
                                            </a>
                                        @endcan
                                        @if (auth()->user()->can('edit-tasks', $team_task))
                                            <a href="{{ route('tasks.edit', ['task' => $team_task]) }}">
                                                <button class="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-xs px-2 py-1 rounded-full shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-150" type="button">
                                                    <i class="fas fa-pen"></i> {{ __('Edit') }}
                                                </button>
                                            </a>
