        ...
        <div class="relative flex flex-col min-w-0 break-words">
            <div class="flex-auto text-white text-xl font-semibold">
                <div class="lg:w-1/2 w-full mb-6 lg:mb-0">
                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-1 text-white">{{ __("{$user->name} Projects") }}</h1>
                    ...
                    @forelse($projects as $project)
                        <tr>
                            ...
                            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-center">
                                <a href="{{ route('users.tasks.index', ['user' => $user]) }}" class="hover:underline">{{ $project->tasks_count }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                {{ __('No Record Found') }}
                            </td>
                        </tr>
                    @endforelse
