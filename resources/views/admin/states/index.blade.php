        ...
        <div class="relative flex flex-col min-w-0 break-words">
            <div class="flex-auto text-white text-xl font-semibold">
                <div class="lg:w-1/2 w-full mb-6 lg:mb-0">
                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-2 text-white">{{ __('Project States') }}</h1>
                ...
                <table class="items-center w-full bg-transparent border-collapse">
                    ...
                    <tbody>
                        @forelse ($states as $state)
                            <tr>
                                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 hover:underline hover:cursor-pointer" onclick="toggleModal('small-modal-id', '{{ $state->name }}', '{{ $state->description }}', '{{ route('admin.states.edit', ['state' => $state]) }}')">
                                    {{ $state->name }}
                                </td>
