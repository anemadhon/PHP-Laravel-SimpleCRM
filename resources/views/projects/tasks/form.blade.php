    ...
    <div class="px-4 w-full pb-2 md:pb-4">
        <div class="flex relative flex-col mb-6 min-w-0 break-words bg-white rounded shadow-lg bg-blueGray-100 xl:mb-0">
            ...
            <div class="px-6 py-6 mb-0 bg-white rounded-t">
                <div class="flex justify-between text-center">
                    <h6 class="text-xl font-bold text-blueGray-700">
                        {{ __("{$state} {$project->name} Tasks Form") }}
                    </h6>
                </div>
            </div>
            ...
            <div class="flex-auto p-4">
                ...
                <form action="{{ $state === 'Update' ? route('projects.tasks.update', ['project' => $project, 'task' => $task]) : route('projects.tasks.store', ['project' => $project]) }}" method="POST">
                    @csrf
                    @if ($state === 'Update')
                        @method('PUT')
                    @endif
